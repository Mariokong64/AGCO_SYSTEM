...
$(document).ready(function () {
  $(".tablaInventarioPC").DataTable({
    processing: true,
    serverSide: true,
    sAjaxSource: "serverside/inventarioPC.serverside.php",
    columnDefs: [
...

     {
        ...
        data: null,
        defaultContent:
        "<div class='btn-group'><button class='btn btn-warning btnEditarAF' idInventario='' data-toggle='modal' data-target='#modalEditarAF' style='margin-left: 10px;'><i class='fa fa-pencil'></i></button><button class='btn btn-danger btnEliminarAF' idInventario='' style='margin-left: 5px;'><i class='fa fa-trash-o'></i></button></div>",
      ...
      //Aquí vamos a hacer que en la octava columna aparezca como "No asignado" en dado caso de que venga un valor nulo.
    ...
    createdRow: function (row, data, dataIndex) {
      ...
      $(row)
        ...
        .on("click", function () {
          ...
          $.ajax({
            url: "ajax/inventarioPC.ajax.php",
            method: "POST",
            data: datos,
            cache: false,
            contentType: false,
      ...
      $(row)
        ...
        .on("click", function () {
          ...
          swal({
            ...
            if (result.value) {
              window.location =
              "index.php?ruta=inventarioPC&idInventario=" + idInventario;
            }
          });
        });
...
$("#filtroSeriePC").keyup(function () {
  var table = $("#tablaInventarioPC").DataTable();
  table.column($(this).data("index")).search(this.value).draw();
});

//Filtro para el modelo del activo fijo

$("#filtroModeloPC").keyup(function () {
  var table = $("#tablaInventarioPC").DataTable();
  table.column($(this).data("index")).search(this.value).draw();
});
...
//Filtro para si el activo fijo esta asignado o no

$("#filtroAsignadoPC").on("keyup", function () {
  var table = $("#tablaInventarioPC").DataTable();
  var searchValue = this.value.toLowerCase();
...
$("#filtroDepartamentoPC").keyup(function () {
  var table = $("#tablaInventarioPC").DataTable();
  table.column($(this).data("index")).search(this.value).draw();
});
...
$("#filtroUbicacionPC").keyup(function () {
  var table = $("#tablaInventarioPC").DataTable();
  table.column($(this).data("index")).search(this.value).draw();
});
...
$("#filtroEmpleadoPC").keyup(function () {
  var table = $("#tablaInventarioPC").DataTable();
  table.column($(this).data("index")).search(this.value).draw();
});
...
$("#filtroEstatusPC").keyup(function () {
  var table = $("#tablaInventarioPC").DataTable();
  table.column($(this).data("index")).search(this.value).draw();
});

//Filtro para saber el hostname del activo fijo

$("#filtroHostPC").keyup(function () {
  var table = $("#tablaInventarioPC").DataTable();
  table.column($(this).data("index")).search(this.value).draw();
});
...
$("#filtroDetallesPC").keyup(function () {
  var table = $("#tablaInventarioPC").DataTable();
  table.column($(this).data("index")).search(this.value).draw();
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var botonReporteComputadoras = document.getElementById("reporteComputadoras");

  if (botonReporteComputadoras) {
    botonReporteComputadoras.addEventListener("click", function () {
      var table = $("#tablaInventarioPC").DataTable();
      ...
      setTimeout(function () {
        ...
        XLSX.writeFile(wb, "Reporte-PC.xlsx");
      }, 2000);
    });
  }
});
...
$("#btnImprimirReportePC").click(function () {
  var table = $("#tablaInventarioPC").DataTable();
  var nombreReporte = $("#NombreReporteDeActivosPC").val();
  var series = [];
  ...
  table.on("draw.dt", function () {
    ...
    setTimeout(function () {
      ...
      window.open("extensiones/TCPDF-main/reportes/inventarioPC-reporte.php?nombreReporte="+nombreReporte+"&series="+series, "_blank");
